...
def handle_disconnect():
    ...
    if my_id and sid and my_id in online_users:
        ...
        if not online_users[my_id]:
            if my_id in online_users: # Sicherstellen, dass der Key noch existiert
...
def handle_set_color():
    ...
    db = load_data()
    if my_id in db['users']:
        new_color = f'hsl({random.randint(0, 360)}, 100%, 75%)' # HSL statt fester Farben
...
def handle_load_chat(payload):
    ...
    if room_data:
        ...
        # Fallback, falls der Raum (noch) nicht existiert
        emit('chat_history', {'room': room_id, 'messages': []})
        join_room(room_id)
...
if __name__ == '__main__':
    ...
    # WICHTIG: Render benutzt den 'Procfile', nicht diesen Befehl.
    # Dieser 'if'-Block ist nur für lokales Testen.
